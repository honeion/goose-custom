...
use goose::config::{
    Config, ConfigError, ExperimentManager, ExtensionEntry, GooseMode,
    PermissionManager,
};
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    ...
    // 내부망 버전: 수동 설정만 지원 (외부 OAuth 서비스 제거)
...
// handle_openrouter_auth, handle_tetrate_auth 제거됨 (외부 서비스)
...
fn add_provider() -> anyhow::Result<()> {
    // 내부망 버전: OpenAI Compatible + Ollama만 지원
    let provider_type = cliclack::select("What type of API is this?")
        .item(
            "openai_compatible",
            "OpenAI Compatible",
            "Uses OpenAI API format (Azure OpenAI, vLLM, LocalAI 등)",
        )
        .item(
            "ollama_compatible",
            "Ollama Compatible",
